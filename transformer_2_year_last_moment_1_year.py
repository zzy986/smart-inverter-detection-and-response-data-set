...
# 4. 按月份划分数据集
train_mask = x_data_combined['Timestamp'].dt.month <= 8  # 前8个月为训练集
valid_mask = (x_data_combined['Timestamp'].dt.month > 8) & (x_data_combined['Timestamp'].dt.month <= 10)  # 9-10月为验证集
test_mask = x_data_combined['Timestamp'].dt.month > 10  # 11-12月为测试集
...
class EarlyStopping:
    def __init__(self, patience=30, delta=0, save_path='best_model_last_moment_1_year.pth'):
...
def train_model(model, train_loader, valid_loader, optimizer, criterion, epochs, early_stopping):
    ...
    model.load_state_dict(torch.load(f'{model_name}_best_model_last_moment_1_year.pth'))
...
for model_name, model in models.items():
    ...
    early_stopping = EarlyStopping(patience=30, save_path=f'{model_name}_best_model_last_moment_1_year.pth')
...
for model_name, y_pred in y_pred_np.items():
    ...
    print(f"\n{model_name} Performance on Test Data_1_year_last_moment:")
