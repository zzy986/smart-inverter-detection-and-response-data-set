...
"""
models = {'Transformer': TransformerRegressor(input_dim=X_train_tensor.shape[2], output_dim=y_train_tensor.shape[1]).to(
        device),
...
class EarlyStopping:
    def __init__(self, patience=30, delta=0, save_path='best_model_1A.pth'):
        """
...
def train_and_evaluate_model(model, train_loader, valid_loader, test_loader, optimizer, criterion):
    ...
    # 初始化早停法
    early_stopping = EarlyStopping(patience=30, save_path=f'{model.__class__.__name__}_best_model_1A.pth')
    ...
    model.load_state_dict(torch.load(f'{model.__class__.__name__}_best_model_1A.pth'))
...
class MAPELoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        # 防止除零错误，通常我们会加一个 epsilon 来避免真实值为0的情况
        epsilon = 1e-8
        loss = torch.mean(torch.abs((target - pred) / (target + epsilon))) * 100
        return loss
...
plt.legend()
plt.savefig('Training_loss_1A.pdf')
plt.show()
...
plt.legend()
plt.savefig('Validation_loss_1A.pdf')
plt.show()
