...
# 假设 x_data_2022 和 y_data_2022 已经包含数据
...
# 1. 确保 timestamp 是 datetime 格式，并按时间排序
...
# 2. 筛选前半年（1月到6月）的数据
mask_half_year = x_data_combined['Timestamp'].dt.month <= 6
x_data_half_year = x_data_combined[mask_half_year]
y_data_half_year = y_data_combined[mask_half_year]

# 3. 划分训练、验证和测试集
train_mask = x_data_half_year['Timestamp'].dt.month <= 4  # 1-4月为训练集
valid_mask = x_data_half_year['Timestamp'].dt.month == 5  # 5月为验证集
test_mask = x_data_half_year['Timestamp'].dt.month == 6   # 6月为测试集

# 4. 提取各部分数据
X_train = x_data_half_year[train_mask][['Module_Temperature_degF', 'Ambient_Temperature_degF',
                                        'Solar_Irradiation_Wpm2', 'Wind_Speed_mps']].values
y_train = y_data_half_year[train_mask][['DC_Voltage', 'DC_Current']].values

X_valid = x_data_half_year[valid_mask][['Module_Temperature_degF', 'Ambient_Temperature_degF',
                                        'Solar_Irradiation_Wpm2', 'Wind_Speed_mps']].values
y_valid = y_data_half_year[valid_mask][['DC_Voltage', 'DC_Current']].values

X_test = x_data_half_year[test_mask][['Module_Temperature_degF', 'Ambient_Temperature_degF',
                                      'Solar_Irradiation_Wpm2', 'Wind_Speed_mps']].values
y_test = y_data_half_year[test_mask][['DC_Voltage', 'DC_Current']].values
...
class EarlyStopping:
    def __init__(self, patience=30, delta=0, save_path='best_model_half.pth'):
...
def train_model(model, train_loader, valid_loader, optimizer, criterion, epochs, early_stopping):
    ...
    model.load_state_dict(torch.load(f'{model_name}_best_model_half.pth'))
...
for model_name, model in models.items():
    ...
    early_stopping = EarlyStopping(patience=30, save_path=f'{model_name}_best_model_half.pth')
...
for model_name, y_pred in y_pred_np.items():
    ...
    print(f"\n{model_name} Performance on Test Data_half_sensor:")
    ...
    plt.title(f'{model_name} - Voltage (V) Prediction_sensor')
    ...
    plt.title(f'{model_name} - Current (I) Prediction_half_year_sensor')
    ...
    plt.title(f'{model_name} - Voltage (V) Time Series_half_year_sensor')
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f'{model_name} - Current (I) Time Series_half_year')
    plt.legend()
    plt.tight_layout()
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training Loss for Different Models_half')
plt.legend()
