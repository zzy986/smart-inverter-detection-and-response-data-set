...
# 3. 添加上一时刻的电压和电流作为特征，并用零值填充第一行
y_data_half_year['DC_Voltage_Lag'] = y_data_half_year['DC_Voltage'].shift(1, fill_value=0)
y_data_half_year['DC_Current_Lag'] = y_data_half_year['DC_Current'].shift(1, fill_value=0)

# 4. 构建特征矩阵 X，加入滞后特征
X = np.stack([x_data_half_year['Module_Temperature_degF'].values,
              x_data_half_year['Ambient_Temperature_degF'].values,
              x_data_half_year['Solar_Irradiation_Wpm2'].values,
              x_data_half_year['Wind_Speed_mps'].values,
              y_data_half_year['DC_Voltage_Lag'].values,
              y_data_half_year['DC_Current_Lag'].values], axis=1)

# y 仍然是当前时刻的电压和电流
y = np.stack([y_data_half_year['DC_Voltage'].values, y_data_half_year['DC_Current'].values], axis=1)
...
class EarlyStopping:
    def __init__(self, patience=30, delta=0, save_path='best_model_last_moment_half.pth'):
...
def train_model(model, train_loader, valid_loader, optimizer, criterion, epochs, early_stopping):
    ...
    model.load_state_dict(torch.load(f'{model_name}_best_model_last_moment_half.pth'))
...
for model_name, model in models.items():
    ...
    early_stopping = EarlyStopping(patience=30, save_path=f'{model_name}_best_model_last_moment_half.pth')
...
for model_name, y_pred in y_pred_np.items():
    ...
    print(f"\n{model_name} Performance on Test Data_half_last_moment:")
